...
import os
from move_with_angle_17kps import exploration


def get_rdm_movement():
    x_rand = np.zeros((17,1)) + 100*np.random.randn(1)
    y_rand = np.zeros((17,1)) + 100*np.random.randn(1)
    z_rand = np.zeros((17,1)) + 100*np.random.randn(1)
...
out = cv2.VideoWriter('random_global_movement_and_angle_rotation.mp4', cv2.VideoWriter_fourcc(*'XVID'), 12, (1920, 640))
...
while True:
    ...
    random_angles_dict = {i: [np.random.normal(scale=maximum_angle), np.random.normal(scale=maximum_angle),
                              np.random.normal(scale=maximum_angle)] for i in range(10)}
    random_kps = exploration(random_kps, random_angles_dict)
    random_kps = random_kps.reshape(17, 3)
    random_kps = random_kps + get_rdm_movement()
    ...
    cv2.imshow("2d projection", img_2d)
